...
public final class ArrayType<E extends ABIType<?>, J> extends ABIType<J> {
    ...
    private int calcElementsLen(Object value) {
        return switch (elementType.typeCode()) {
            ...
            case TYPE_CODE_BIG_INTEGER, TYPE_CODE_BIG_DECIMAL -> ((Number[]) value).length * UNIT_LENGTH_BYTES;
            case TYPE_CODE_ARRAY, TYPE_CODE_TUPLE -> measureByteLength((Object[]) value);
            case TYPE_CODE_ADDRESS -> ((Address[]) value).length * UNIT_LENGTH_BYTES;
    ...
    int byteLengthPacked(Object value) {
        ...
        return switch (elementType.typeCode()) {
            case TYPE_CODE_BOOLEAN -> ((boolean[]) value).length; // * 1
            case TYPE_CODE_BYTE -> byteCount(value); // * 1
            case TYPE_CODE_INT -> ((int[]) value).length * elementType.byteLengthPacked(null);
            case TYPE_CODE_LONG -> ((long[]) value).length * elementType.byteLengthPacked(null);
            case TYPE_CODE_BIG_INTEGER, TYPE_CODE_BIG_DECIMAL -> ((Number[]) value).length * elementType.byteLengthPacked(null);
            case TYPE_CODE_ARRAY, TYPE_CODE_TUPLE, TYPE_CODE_ADDRESS -> measureByteLengthPacked((Object[]) value);
            default -> throw new AssertionError();
        };
    }
    ...
    private int validateElements(J value) {
        return switch (elementType.typeCode()) {
            ...
            case TYPE_CODE_BIG_INTEGER, TYPE_CODE_BIG_DECIMAL, TYPE_CODE_ARRAY, TYPE_CODE_TUPLE, TYPE_CODE_ADDRESS -> validateObjects((Object[]) value);
            default -> throw new AssertionError();
        };
    }
    ...
    J decode(ByteBuffer bb, byte[] unitBuffer) {
        ...
        return switch (elementType.typeCode()) {
            case TYPE_CODE_BOOLEAN -> (J) decodeBooleans(arrayLen, bb, unitBuffer);
            case TYPE_CODE_BYTE -> (J) decodeBytes(arrayLen, bb);
            case TYPE_CODE_INT -> (J) decodeInts(arrayLen, bb, (IntType) elementType, unitBuffer);
            case TYPE_CODE_LONG -> (J) decodeLongs(arrayLen, bb, (LongType) elementType, unitBuffer);
            case TYPE_CODE_BIG_INTEGER, TYPE_CODE_BIG_DECIMAL, TYPE_CODE_ARRAY, TYPE_CODE_TUPLE, TYPE_CODE_ADDRESS -> (J) decodeObjects(arrayLen, bb, unitBuffer);
            default -> throw new AssertionError();
        };
    }
