...
final class PackedDecoder {
    ...
    private static int decodeTuple(TupleType tupleType, byte[] buffer, int start, int end, Object[] parentElements, int pei) {
        ...
        for (int i = tupleType.size() - 1; i >= 0; i--) {
            ...
            case TYPE_CODE_TUPLE -> end -= decodeTupleStatic((TupleType) type, buffer, end - type.byteLengthPacked(null), end, elements, i);
            default -> end -= decode(tupleType.get(i), buffer, end - type.byteLengthPacked(null), end, elements, i);
            }
        }
    ...
    private static int decode(ABIType<?> type, byte[] buffer, int idx, int end, Object[] elements, int i) {
        ...
        case TYPE_CODE_BOOLEAN -> {elements[i] = BooleanType.decodeBoolean(buffer[idx]); yield type.byteLengthPacked(null);}
        case TYPE_CODE_BYTE -> {elements[i] = buffer[idx]; yield type.byteLengthPacked(null);}
        case TYPE_CODE_INT -> insertInt((IntType) type, buffer, idx, type.byteLengthPacked(null), elements, i);
        case TYPE_CODE_LONG -> insertLong((LongType) type, buffer, idx, type.byteLengthPacked(null), elements, i);
        case TYPE_CODE_BIG_INTEGER -> insertBigInteger((BigIntegerType) type, type.byteLengthPacked(null), buffer, idx, elements, i);
        case TYPE_CODE_BIG_DECIMAL -> insertBigDecimal((BigDecimalType) type, type.byteLengthPacked(null), buffer, idx, elements, i);
        case TYPE_CODE_ARRAY -> insertArray((ArrayType<? extends ABIType<?>, ?>) type, buffer, idx, end, elements, i);
        case TYPE_CODE_TUPLE -> type.dynamic
                    ? decodeTuple((TupleType) type, buffer, idx, end, elements, i)
                    : decodeTupleStatic((TupleType) type, buffer, idx, end, elements, i);
        case TYPE_CODE_ADDRESS -> insertAddress(type.byteLengthPacked(null), buffer, idx, elements, i);
...
//            unscaled = new BigInteger(Arrays.copyOfRange(buffer, idx, idx + elementLen));
    ...
    private static int insertArray(ArrayType<? extends ABIType<?>, ?> arrayType, byte[] buffer, int idx, int end, Object[] dest, int destIdx) {
        ...
        dest[destIdx] = switch (elementType.typeCode()) {
            case TYPE_CODE_BOOLEAN -> decodeBooleanArray(arrayLen, buffer, idx);
            case TYPE_CODE_BYTE -> decodeByteArray(arrayType, arrayLen, buffer, idx);
            case TYPE_CODE_INT -> decodeIntArray((IntType) elementType, elementByteLen, arrayLen, buffer, idx);
            case TYPE_CODE_LONG -> decodeLongArray((LongType) elementType, elementByteLen, arrayLen, buffer, idx);
            case TYPE_CODE_BIG_INTEGER -> decodeBigIntegerArray((BigIntegerType) elementType, elementByteLen, arrayLen, buffer, idx);
            case TYPE_CODE_BIG_DECIMAL -> decodeBigDecimalArray((BigDecimalType) elementType, elementByteLen, arrayLen, buffer, idx);
            case TYPE_CODE_ARRAY, TYPE_CODE_TUPLE, TYPE_CODE_ADDRESS -> decodeObjectArray(arrayLen, elementType, buffer, idx, end);
            default -> throw new AssertionError();
        };
