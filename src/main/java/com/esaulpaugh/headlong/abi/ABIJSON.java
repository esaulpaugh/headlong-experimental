...
//    private static final String NONPAYABLE = "nonpayable";// to mark as nonpayable, do not specify any stateMutability
    ...
    private static ABIObject parseABIObject(TypeEnum t, JsonObject object, MessageDigest digest) {
        return switch (t.ordinal()) {
            case TypeEnum.ORDINAL_FUNCTION, TypeEnum.ORDINAL_RECEIVE, TypeEnum.ORDINAL_FALLBACK, TypeEnum.ORDINAL_CONSTRUCTOR -> _parseFunction(t, object, digest);
            case TypeEnum.ORDINAL_EVENT -> _parseEvent(object);
            case TypeEnum.ORDINAL_ERROR -> _parseError(object);
